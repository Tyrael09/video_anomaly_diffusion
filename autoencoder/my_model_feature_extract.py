...
from my_data_load import VideoDataset, VideoClipDataset
...
def load_resnet3D(depth=50):
    ...
    PATH = f'video_anomaly_diffusion/autoencoder/ResNext_models/trained/r3d{depth}_KM_200ep.pth'
...
def main_shangai():
    ...
    with torch.no_grad():
        ...
        ds = VideoDataset(root_dir=root_dir,
                          dataset_name='regular_videos_long/test/')
        dst_data_path = ds.path + f'_{feat_model}_vfeat'

        mkdir(dst_data_path)
        check_gpu() # check whether (correct) GPU is available
        ...
        for idx in trange(len(ds)):
            ...
            for i in range(0,len(batched),batch_size):
                ...
                x = x.cuda(gpu_id)
                out = model(x)
                outs.append(out.cpu()) # causing high CPU usage??
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        ...
        root_dir = '/local/scratch/Cataract-1K-Hendrik/irregular_videos'
